...
/**
 ...
 struct LanguageModelConfiguration: Sendable, Identifiable, Hashable {
     ...
     /// For dynamic-shape backends (MLX, llama.cpp) this is just a suggested cap.
     ...
     /// Bundle filenames (with extension) that the tokenizer needs.
 ...
 struct GenerationOptions: Sendable {
     ...
     /// Kept separate from hard EOS tokens in model config.
 ...
 enum TokenizerFactory {
     static func make(for config: LanguageModelConfiguration) async throws -> any TokenizerProtocol {
         ...
         var missing: [String] = []
         for fname in config.tokenizerFiles {
 ...
 struct PlainPromptTemplate: PromptTemplate {
     ...
     func format(messages: [ChatMessage], context: [RetrievedChunk]) -> String {
         ...
         if let firstCtx = context.first?.text {
             // Keep the app's original "The Bible says about X: … This means" shape.
 ...
 /// Core ML backend — runs DistilGPT2 and any fixed-shape Core ML export
 /// that exposes `input_ids`, `position_ids`, `output_logits`.
 actor CoreMLLanguageModel: LanguageModel {
     ...
     func predictNextToken(contextTokens ids: [Int],
         ...
         let seq = Array(ids.suffix(padLen))
         let n = seq.count
         ...
         for i in 0..<padLen {
             input[i] = NSNumber(value: i < n ? seq[i] : padID)
         ...
         else { return configuration.fallbackTokenId }

         // Expected shape: [1, seqLen, vocab, 1, 1]
         ...
         // Extract logits slice into a plain [Double]
         var raw = [Double](repeating: -.infinity, count: vocab)
         for v in 0..<vocab {
             let idx = baseIdx + v
             if idx < logits.count { raw[v] = logits[idx].doubleValue }
         }

         return sampler.sample(
             logits: raw,
             alreadyGenerated: Set(generated),
             options: options
         )
     }
 }
 ...
 */
 ...
 extension LanguageModelConfiguration {
     ...
     static let distilGPT2 = LanguageModelConfiguration(
         ...
         backend: .coreML,
         contextLength: 1024,
         maxInputTokens: 64,          // matches the existing Core ML export's fixed shape
         vocabSize: 50257,
         ...
         stopTokenIds: [50256],        // <|endoftext|>
         padTokenId: 50256,
         fallbackTokenId: 198,         // "\n" — used when logits unreadable
         defaultOptions: GenerationOptions(
             ...
             softStopTokens: [13]       // "." for one-sentence summaries
         )
     )

     /// Llama 3.2 1B-Instruct — needs MLX backend (see MLXLanguageModel TODO).
     /// Switching to this model in the UI will show an actionable error until MLX is wired.
     static let llama3_2_1bInstruct = LanguageModelConfiguration(
         id: "llama-3.2-1b-instruct-4bit",
         displayName: "Llama 3.2 1B Instruct (MLX, 4-bit)",
         architecture: .llama3,
         backend: .mlx,
         contextLength: 131_072,       // 128K context
         maxInputTokens: 8_192,        // practical cap per turn; MLX KV cache handles more
         vocabSize: 128_256,
         weightsResourceName: "Llama-3.2-1B-Instruct-4bit",
         weightsResourceExtension: "",
         tokenizerFiles: [
             "tokenizer.json", "tokenizer_config.json"
         ],
         promptTemplateID: "llama3",
         // Llama 3.2 terminators: <|end_of_text|>=128001, <|eom_id|>=128008, <|eot_id|>=128009
         stopTokenIds: [128001, 128008, 128009],
         padTokenId: 128004,           // <|finetune_right_pad_id|>
         ...
         defaultOptions: GenerationOptions(
             ...
             softStopTokens: []        // no soft stop — let the chat template's EOT handle it
         )
     )
 }
         ...
         AsyncThrowingStream { [retriever, model, tokenizer, template, sampler] continuation in
             let task = Task {
                 do {
                     ...
                     let prompt = template.format(messages: messages, context: chunks)
         ...
         task = Task { [weak self] in
             do {
                 let stream = rag.answer(
                     query: topic,
                     systemPrompt:
                         "You are a thoughtful assistant who explains Bible passages clearly and reverently. " +
                         "Ground your answer in the retrieved verses.",
                     k: 3,
